...
function checkTorrentDay() {
	echo '***********************'
	echo "Checking torrentday for ${SEARCHPARAMETERS//\|/, }"
	echo '***********************'
	...
	do
			TITLE=$( less "$TMPFILETORRENTDAY" | grep '<title>' | sed -n "${TITLEROW}p" | awk -F'<title>' '{print $2}' | awk -F'</title>' '{print $1}' )
			LINK=$( less "$TMPFILETORRENTDAY" | grep '<link>' | sed -n "${TITLEROW}p" | awk -F'<link>' '{print $2}' | awk -F'</link>' '{print $1}' )
			echo "FOUND $TITLE"
...
function checkTorrentLeech() {
	echo '***********************'
	echo "Checking torrentleech for ${SEARCHPARAMETERS//\|/, }"
	echo '***********************'
	...
	for TITLEROW in "${TITLEROWS[@]}"
	do
			TITLE=$( less "$TMPFILETORRENTLEECH" | grep '<title>' | sed -n "${TITLEROW}p" | awk -F'[' '{print $3}' | awk -F']' '{print $1}' )
			LINK=$( less "$TMPFILETORRENTLEECH" | grep '<link>' | sed -n "${TITLEROW}p" | awk -F'[' '{print $3}' | awk -F']' '{print $1}' )
			echo "FOUND $TITLE"
			echo "TORRENTTITLE: $TITLE" >> "$TMPFILE"
...
function checkPirateBay() {
	echo '***********************'
	echo "Checking piratebay for ${SEARCHPARAMETERS//\|/, }"
	echo '***********************'
	...
	for TITLEROW in "${TITLEROWS[@]}"
	do
			TITLE=$( less "$TMPFILEPIRATEBAY" | grep '<title>' | sed -n "${TITLEROW}p" | awk -F'[' '{print $3}' | awk -F']' '{print $1}' )
			LINK=$( less "$TMPFILEPIRATEBAY" | grep '<link>' | sed -n "${TITLEROW}p" | awk -F'<link>' '{print $2}' | awk -F'</link>' '{print $1}' )
			echo "FOUND $TITLE"
			echo "TORRENTTITLE: $TITLE" >> "$TMPFILE"
			echo "TORRENTLINK: $LINK" >> "$TMPFILE"
	done
...
function checkKickass() {
	echo '***********************'
	echo "Checking KickassTorrents for ${SEARCHPARAMETERS//\|/, }"
	echo '***********************'
	...
	for TITLEROW in "${TITLEROWS[@]}"
	do
			TITLE=$( less "$TMPFILEKICKASS" | grep '<title>' | sed -n "${TITLEROW}p" | awk -F'<title>' '{print $2}' | awk -F'</title>' '{print $1}' )
			LINK=$( less "$TMPFILEKICKASS" | grep '<torrent:magnetURI>' | sed -n "${TITLEROW}p" | awk -F'[' '{print $3}' | awk -F']' '{print $1}' )
			echo "FOUND $TITLE"
			echo "TORRENTTITLE: $TITLE" >> "$TMPFILE"
			echo "TORRENTLINK: $LINK" >> "$TMPFILE"
	done
...
function chooseTorrent() {
	if [[ ! $( less "$TMPFILE" | wc -l ) -eq 1 ]]; then
		...
		ROWS=$( less "$TMPFILE" | grep -c 'TORRENTTITLE: ' )
		for ((i = 1; i <= ROWS; i++))
		do
			TITLE=$( less "$TMPFILE" | grep 'TORRENTTITLE: ' | sed -n "${i}p" | awk -F'TORRENTTITLE: ' '{print $2}' )
			echo "$i: $TITLE "
		done
		read -r INPUT
		if [[ ! -z $INPUT ]]; then
...
function start() {
	...
	case $ARGUMENTS in
		"*$HELPER*" )
			...
			;;
		"*$REPEATER*" )
			ARGUMENTWITHOUTCMD="${ARGUMENTS//--repeat/}"
